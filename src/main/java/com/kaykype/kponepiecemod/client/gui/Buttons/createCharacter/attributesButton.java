...
public class attributesButton extends Button {
    ...
    boolean isAttrButton;

    public attributesButton (int x, int y, int width, int height, String text, IPressable onPress, boolean isAttrButton) {
        super(x, y, width, height, new StringTextComponent(text), onPress);
        this.isAttrButton = isAttrButton;
    }

    public void setIsAttr(boolean isAttr) {this.isAttrButton = isAttr;}
    @Override
    public void renderButton(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks){
        int textColor = isAttrButton ? 0xbababa : 0xFFFFFF;
        ...
        fontRenderer.drawShadow(matrixStack, "Atributos", (x-(mc.font.width("Atributos")/2))+(width/2), (y+(height/2))-(mc.font.lineHeight/2), textColor);
    }
}
